...
root_path = '/pfs/mt-1oY5F7/chenshuo/sparse_test/run_results/our_sparse_new'
datasets = ["dl3dv", "llff", "mipnerf360", "TanksandTemples"]
gpu_count = 4

# Define the common parameters
method = "our_demo_new_sparse"
val_length = 12
lambda_lpips = 0.4
use_confidence = "true"
...
for dataset in datasets:
    ...
    case_videos = glob.glob(os.path.join(dataset_path, "*.mp4"))
    for case_video in case_videos:
        case_name = os.path.splitext(os.path.basename(case_video))[0]
        all_cases.append((dataset, case_name, case_video))

def process_case(args):
    gpu_id, dataset, case_name, case_video = args
    ...
    # Commands
    commands = [
        f"python get_frame.py {case_video} {dataset_dir} 30",
