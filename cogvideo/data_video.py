...
class SFTDataset(Dataset):
    def __init__(self, data_dir, video_size, fps, max_num_frames, output_dir, caption_dir, skip_frms_num=3):
        """
        skip_frms_num: ignore the first and the last xx frames, avoiding transitions.
        """
        ...
        with open(caption_dir, "r", encoding="utf-8") as file:
            ...
            for row in reader:
                if len(row) == 2:  
                    video_name = os.path.join(data_dir, row[0]) 
                    caption = row[1].strip()
    ...
    def get_uniform_indices(self, start: int, end: int, num_frames: int):
        """
        Select indices uniformly from the range [start, end), distributing the indices as evenly as possible,
        ensuring that step+1 is used exactly `remainder` times.
        
        Args:
            start (int): The starting point of the range (inclusive).
            end (int): The ending point of the range (exclusive).
            num_frames (int): The total number of indices to select.

        Returns:
            np.ndarray: A NumPy array of selected indices, with type int.
        """
        # Calculate base step size and the remainder
        total_length = end - start
        step = total_length // num_frames
        remainder = total_length % num_frames
        
        indices = []
        current_pos = start
        extra_step_positions = np.linspace(0, num_frames, remainder, endpoint=False, dtype=int)

        for i in range(num_frames):
            indices.append(current_pos)
            if i in extra_step_positions:
                current_pos += step + 1
            else:
                current_pos += step
    ...
    def log_videos(self, temp_frms, output_dir, name):
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        gif_frames = []
        for frame in temp_frms:
            # frame = rearrange(frame, "c h w -> h w c")
            frame = (1.0 * frame).cpu().numpy().astype(np.uint8)
            gif_frames.append(frame)
            # print(frame.shape)
        now_save_path = os.path.join(output_dir, f"{name}.mp4")
        with imageio.get_writer(now_save_path, fps=8) as writer:
            try:
                for i, frame in enumerate(gif_frames):
                    writer.append_data(frame)
                    print(f"Written frame {i}")
            except Exception as e:
                print(f"Error while writing video: {e}")
            finally:
                writer.close()
    ...
    def __getitem__(self, index):
        ...
        if ori_vlen / actual_fps * self.fps > self.max_num_frames:
            ...
            batch_indices = self.get_uniform_indices(start, end, num_frames)
            temp_frms = vr.get_batch(batch_indices)
            # self.log_videos(temp_frms, self.output, self.captions[index][:100])
            assert temp_frms is not None
            tensor_frms = torch.from_numpy(temp_frms) if type(temp_frms) is not torch.Tensor else temp_frms
            # tensor_frms = tensor_frms[torch.tensor((batch_indices - start).tolist())]
        else:
...
if __name__ == "__main__":
    dataset = SFTDataset('/pfs/mt-1oY5F7/chenshuo/circle_video/test.csv',
                          [ 480, 720 ], 8, 49, skip_frms_num=3)
